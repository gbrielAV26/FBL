...
import { OFFICE_INFO, PRACTICE_AREAS, DEPARTMENT_CONTACTS, ADMINISTRATIVE_TEAM } from '../data/fblData';
import { Language } from '../types';
import { MapPin, Phone, Mail, Clock, Send, CheckCircle2, Briefcase, Building2, FileText, Receipt, Landmark, Archive, UserCheck } from 'lucide-react';
  ...
  return (
    <div className="py-16 md:py-24 px-5 md:px-12 lg:px-16 bg-[#fbf9f9] min-h-screen">
      <div className="max-w-[1280px] mx-auto">
        ...
        {/* Departamentos & Serviços Administrativos */}
        <div className="mt-16 pt-12 border-t border-[#1b1c1c]/10">
          <div className="flex items-center gap-3 mb-2">
            <div className="h-[2px] w-8 bg-[#9e0418]"></div>
            <span className="text-xs uppercase tracking-widest font-bold text-[#9e0418]">
              {language === 'pt' ? 'Contactos por Departamento' : 'Direct Department Contacts'}
            </span>
          </div>
          <h3 className="font-serif-headline text-2xl font-bold text-[#1b1c1c] mb-2">
            {language === 'pt' ? 'Serviços Administrativos, Facturação & Expediente' : 'Administrative Services, Billing & Case Records'}
          </h3>
          <p className="text-sm text-[#5e5e5e] max-w-2xl mb-8">
            {language === 'pt'
              ? 'Para assuntos específicos de gestão documental, emissão de facturas, regularização de contas ou secretariado forense, utilize os canais directos:'
              : 'For specific inquiries regarding document records, billing, payments, or forensic secretariat, please contact the dedicated departments:'}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {DEPARTMENT_CONTACTS.map((dept) => (
              <div
                key={dept.id}
                className="bg-white border border-stone-200/90 p-5 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.03)] hover:border-[#9e0418]/30 transition-colors flex flex-col justify-between"
              >
                <div>
                  <div className="w-9 h-9 rounded-sm bg-[#9e0418]/5 border border-[#9e0418]/20 flex items-center justify-center mb-3 text-[#9e0418]">
                    {dept.id === 'secretariado' && <Briefcase className="w-4 h-4" />}
                    {dept.id === 'facturacao' && <Receipt className="w-4 h-4" />}
                    {dept.id === 'cobranca' && <Landmark className="w-4 h-4" />}
                    {dept.id === 'arquivo' && <Archive className="w-4 h-4" />}
                  </div>
                  <h4 className="font-serif-headline text-base font-bold text-[#1b1c1c] leading-snug">
                    {dept.name[language]}
                  </h4>
                ...
                <div className="mt-4 pt-3 border-t border-stone-100">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-[#9e0418] hover:underline break-all"
                  >
          ...
          {/* Equipa de Apoio e Gestão Administrativa */}
          <div className="mt-10 bg-stone-100/70 border border-stone-200/80 p-6 sm:p-7">
            <div className="flex items-center gap-2 mb-4">
              <UserCheck className="w-4 h-4 text-[#9e0418]" />
              <h4 className="font-serif-headline text-base font-bold text-[#1b1c1c]">
                {language === 'pt' ? 'Apoio Operacional & Gestão Administrativa' : 'Operations & Administrative Management Support'}
              </h4>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {ADMINISTRATIVE_TEAM.map((staff) => (
                <div
                  key={staff.id}
                  className="bg-white border border-stone-200 p-4 rounded-xs shadow-xs flex flex-col justify-between"
                >
                  <div>
                    <span className="font-serif-headline font-bold text-sm text-[#1b1c1c] block">
                      {staff.name}
                    </span>
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-mono font-semibold text-[#9e0418] hover:underline mt-3 pt-2 border-t border-stone-100"
                  >
