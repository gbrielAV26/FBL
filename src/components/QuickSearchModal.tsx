...
import { PRACTICE_AREAS, TEAM_MEMBERS, PUBLICATIONS } from '../data/fblData';
import { Language, NavTab } from '../types';
import { Search, X, Briefcase, User, BookOpen, ArrowRight } from 'lucide-react';
...
export const QuickSearchModal: React.FC<QuickSearchModalProps> = ({
  ...
  const results = useMemo(() => {
    if (!query.trim()) return { practiceAreas: [], team: [], publications: [] };
    ...
    return { practiceAreas, team, publications };
  }, [query, language]);

  if (!isOpen) return null;

  const totalResults =
    results.practiceAreas.length + results.team.length + results.publications.length;
